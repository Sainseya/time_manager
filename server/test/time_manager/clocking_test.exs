defmodule Time_manager.ClockingTest do
  use Time_manager.DataCase

  alias Time_manager.Clocking

  describe "clocks" do
    alias Time_manager.Clocking.Clock

    import Time_manager.ClockingFixtures
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~N[2024-10-06 17:24:00]}

      assert {:ok, %Clock{} = clock} = Clocking.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2024-10-06 17:24:00]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2024-10-07 17:24:00]}

      assert {:ok, %Clock{} = clock} = Clocking.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2024-10-07 17:24:00]
    end
